...
> Arrays are section 1; the pointer machinery that makes a linked list possible is sections 2.7
> and 3.
...
byte:  16   17   18   19
      ...
      every element is 4 bytes wide and starts on a multiple of 4,
      so there is no padding: sizeof(arr) = 5 * 4 = 20 bytes
```
...
- RAM is read in **64-byte cache lines** (on x86-64), so one cache miss brings in 16 `int`s and the
  ...
  list**, even though both are **O(n)**. It comes back in sections 2.6 and 3.9.
...
| | Array | Structure |
|---|---|---|
| Element types | all identical | may differ |
| Access | by index, `arr[i]` | by name, `s.gpa` |
| Address of element | computed: `base + i * size` | fixed at compile time (an offset) |
| Size | `n * sizeof(element)` | sum of members **+ padding** |

That last row is where the interesting part is.
...
byte:  16   17   18   19   20   21   22   23
      ...
      ## = padding            sizeof(A) = 24, useful data = 10 bytes
```

Compare that with the array diagram in section 1: there, every element had the same size and the
same alignment, so they packed back-to-back with no gaps and `base + i * size` always landed on a
real element. Here the members have *different* alignments, and the gaps are the price.

Three rules produce that picture:

1. **Each member is placed at the next offset that satisfies its own alignment**, not the
   structure's. `b` is a `double` (`alignof` 8), so it cannot start at offset 1: it is pushed to the
   next multiple of 8, which is offset 8. The 7 skipped bytes become padding.
2. **The alignment of the structure is the largest alignment among its members** - here 8, because
   of the `double`.
3. **`sizeof` is rounded up to a multiple of the structure's alignment** (*tail padding*). Without
   the 7 bytes after `c`, the second element of an `A[2]` would start at offset 17 and its `double`
   would be misaligned.

> **Why the hardware insists on rule 1.** Memory is fetched in aligned blocks, so a 4-byte `int`
> sitting at a multiple of 4 can never straddle a cache line or a page boundary. A misaligned one
> can, turning a single load into two loads plus a merge. On x86-64 that is a slowdown; on some
> architectures (older ARM, SPARC, MIPS) it is an outright fault, and an unaligned load is not
> guaranteed to be atomic. Alignment is a hardware rule the compiler obeys, not a layout preference
> it invents.

> Rule 3 is the one students find arbitrary. The reason is arrays: `sizeof` is exactly the stride
> between consecutive elements, so it must keep **every** element aligned, not just the first.

**The same three rules as an algorithm.** You can work out any layout by hand with this:

```text
offset = 0
for each member, in declaration order:
    round offset up to a multiple of alignof(member)   <- the skipped bytes are padding
    place the member at offset
    offset += sizeof(member)

alignof(struct) = the largest alignof among the members
sizeof(struct)  = round offset up to a multiple of alignof(struct)   <- tail padding
```

Traced on `struct A` above:

| Member | Offset lands on | Padding added |
|---|---|---|
| `char a`, alignof 1 | 0, then offset -> 1 | none |
| `double b`, alignof 8 | 1 is not a multiple of 8, so 8; offset -> 16 | 7 bytes (1..7) |
| `char c`, alignof 1 | 16, then offset -> 17 | none |
| `alignof(A)` = max(1, 8, 1) = **8** | round 17 up to **24** | 7 bytes (17..23) |

> **It is the largest *alignment*, not the largest *member*.** `sizeof(char[100])` is 100 but
> `alignof(char[100])` is 1, so `struct { char buf[100]; int x; }` has alignment 4 and size 104.
> A big member does not make a big alignment.
>
> "Declaration order" is guaranteed for members sharing one access specifier. If you mix `public:`
> and `private:` sections the compiler may reorder between the groups, which does not affect plain
> data structs.
...
#### A worked example: three ways to store a name

The same two fields, written three ways. Only the first member changes:

```c++
struct PersonPtr { char *name;     int age; };   // a pointer to the characters
struct PersonArr { char  name[10]; int age; };   // the characters, stored inline
struct PersonChr { char  name;     int age; };   // a single character
```

| First member | `sizeof` | `alignof` | `offsetof(age)` | Padding |
|---|---|---|---|---|
| `char *name;` | 16 | 8 | 8 | 4, at the **end** |
| `char name[10];` | 16 | 4 | 12 | 2, in the **middle** |
| `char name;` | 8 | 4 | 4 | 3, in the **middle** |

Two traps hide in that table.

**`char *name` is 8 bytes, not 1.** It is a pointer: it holds an address, and the characters live
somewhere else entirely. Every pointer is 8 bytes on x86-64 (section 3.1), so `alignof` is 8, and
this is the only row with tail padding:
...
**`alignof(char[10])` is 1, not 10.** An array's alignment is its *element's* alignment, not its
size, so this struct needs only 4-byte alignment. `age` cannot start at byte 10, so rule 1 pushes it
to 12:
...
The first two are both 16 bytes for entirely different reasons, and they are not the same data
structure: `PersonArr` **contains** the name, `PersonPtr` only points at it.

> **Reordering does not always help.** `struct { int age; char name[10]; }` is still 16 bytes; the
> two wasted bytes just move to the end. The `struct B` trick above pays off when member alignments
> differ a lot. Here the widest is only 4, so there is nothing to reclaim.

You can inspect all of this yourself:

```c++
#include <cstddef>   // offsetof
#include <print>     // C++23; use std::cout if you are on an older standard

std::println("sizeof  = {}", sizeof(A));            // 24
std::println("alignof = {}", alignof(A));           // 8
std::println("offset of b = {}", offsetof(A, b));   // 8
```

> `g++ -Wpadded` warns everywhere the compiler inserts padding. It is noisy, but it is an excellent
> teaching tool - run it once on a struct-heavy file and the layout rules become obvious.
...
This surprises people coming from C: in C++ a `struct` is a **class**. It can have constructors,
destructors, member functions, access specifiers, inheritance and templates. There are exactly two
differences, both about defaults:
...
The common convention: use `struct` for plain data with no invariant to protect, and `class` when
the type maintains an invariant behind a private interface.

### 2.3 Creating and initializing structures
...
### 2.4 Structures in memory, and passing them around
...
### 2.6 Array of structures - and why the reverse sometimes wins
...
### 2.7 Self-referential structures - the bridge to every node-based structure
...
byte:  8    9    10   11   12   13   14   15
      ...
      sizeof(Node) = 16 - 4 bytes of it are padding
```
...
exactly why `Node* next;` works in section 2.7 while `Node next;` cannot.
