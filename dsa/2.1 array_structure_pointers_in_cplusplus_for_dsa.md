...
exactly why `Node* next;` works in section 3.5 while `Node next;` cannot.
...
### 3.5 Self-referential structures - the bridge to every node-based structure
...
```c++
struct Node {
    int   data;   // 4 bytes, then 4 bytes of padding (section 2.1)
...
data structure in this course that is not an array. An array arranges data by putting elements
*next to each other*; a linked list arranges it by having each element *store the address of the
next*, and `Node* next;` is the whole of that second arrangement.

### 3.6 Dynamic memory: `new` and `delete`
...
### 3.9 What to actually use in modern C++

Raw owning pointers are error-prone, and every bug in section 3.6 comes from a human having to
...
### 3.10 Why pointers matter for data structures
...
### 3.11 Array of structures - and why the reverse sometimes wins
...
This maps onto the two versions of list removal from section 3.8. That code used `Node**` - here is
